...
\begin{document}
    ...
    \begin{enumerate}
        ...
        \item Пример разрывной в точке функции, имеющей в ней производную по любому направлению. \textbf{TODO: в чате группы}   
        \item \sout{Дифференцируемость суммы, произведения, композиции. }
        \item \sout{Градиент вещественной функции.} 
        \item \sout{Векторные поля. }
        ...
        \item Высшие дифференциалы. \textbf{TODO: в тетрадке есть оказывается}
        \item Пример не коммутирующих дифференциальных операторов.  
        \item \sout{Теорема о разложении Тейлора.} 
        \item Гессиан вещественной функции. Достаточные условия экстремума. \textbf{TODO: гессиан? что тут про него писать, интересно?}
    ...
    \subsection{Пример разрывной в точке функции, имеющей в ней производную по любому направлению}
        ...
        Т.е. существуют и равны 0.
        Рассмотрим производную по направлению, для этого будем брать производную по вектору $u = (\cos(\alpha), \sin(\alpha))$, который не направлен по осям координат, то есть $\cos(\alpha)\cdot \sin(\alpha) \neq 0$:
        \begin{align*}
        \frac{\partial f}{\partial u}(0, 0) = \lim_{t \to 0} \frac{f(t\cos(\alpha), t\sin(\alpha)) - f(0,0)}{t} = \frac{f(t\cos(\alpha)), t\sin(\alpha)}{t} = \\
        = \lim_{t \to 0} \frac{1}{t}\cdot \frac{t\cos(\alpha)\cdot t\sin(\alpha)}{(t\cos(\alpha))^2 + (t\sin(\alpha))^2} = \lim_{t\to 0} \frac{1}{t} \cdot \frac{t^2 \cdot \cos(\alpha)\sin(\alpha)}{t^2} = \lim_{t\to 0}\frac{1}{t}\cos(\alpha)\sin(\alpha)
        \end{align*}
        Ясно видно, что этот предел равен бесконечности.
        На самом деле, есть теорема, которая гласит "если функция дифференциируема в точке, то она в ней непрерывна и у нее существуют все направленные производные". Условие "существуют производные по любому направлению" на самом деле является
        достаточным признаком непрерывности функции в точке.
